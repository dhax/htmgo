...
import (
	...
	"github.com/dhax/htmgo/cli/htmgo/tasks/util"
	"log"
	"log/slog"
	"os"
	"path/filepath"
...
func DownloadTemplate(outPath string) {
	...
	err := process.Run(process.NewRawCommand("clone-template", "git clone https://github.com/dhax/htmgo --depth=1 "+tempOut, process.ExitOnError))
