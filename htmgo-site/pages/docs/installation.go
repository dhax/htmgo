...
func Installation(ctx *h.RequestContext) *h.Page {
	return DocPage(
		...
		h.Div(
			...
			ui.SingleLineBashCodeSnippet(`GOPROXY=direct go install github.com/dhax/htmgo/cli/htmgo@latest`),
			Text("If you are using Windows, you will need to use the following command instead:"),
			ui.SingleLineBashCodeSnippet(`set GOPROXY=direct && go install github.com/dhax/htmgo/cli/htmgo@latest`),
