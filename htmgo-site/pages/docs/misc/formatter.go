...
func Formatter(ctx *h.RequestContext) *h.Page {
	return DocPage(
		...
		h.Div(
			...
			`),
			HelpText(`Note: if you have previously installed htmgo, you will need to run GOPROXY=direct go install github.com/dhax/htmgo/cli/htmgo@latest to update the cli tool.`),
			Text("Usage:"),
